...
void goto_target() {
  ...
  int interval = abs_r_target / abs_phi_target;
  int i = interval;
  
  if (abs_r_target > abs_phi_target) { // r groter dan phi  -> phi in interval, r constant
    for (long int r = 0; r < abs_r_target; r++) {
      do_r_pulse();
  ...
  } else { // r kleiner dan phi -> r in interval, phi constant
    for (long int phi = 0; phi < abs_r_target; phi++) {
      do_phi_pulse();
